...
import { GraphQLResolveInfo, parse, visit, FieldNode } from 'graphql';
  ...
  fields: () => ({
    ...
    memberType: {
      ...
      resolve: async (parent, _args, context) => {
        return context.loaders.memberType.load(parent.memberTypeId);
      },
    },
  }),
});

const User: GraphQLObjectType<UserSource, ContextValue> = new GraphQLObjectType({
  name: 'User',
  fields: () => ({
    id: { type: new GraphQLNonNull(UUIDType) },
    ...
    profile: { type: Profile,
      resolve: (parent: UserSource, _args, context) => {
    ...
    userSubscribedTo: {
      ...
      resolve: async (parent, _args, context) => {
        ...
        return context.loaders.authors.load(parent.id);
      },
    },
    subscribedToUser: {
      type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(User))),
...
const RootQueryType = new GraphQLObjectType<unknown, ContextValue>({
  ...
  fields: {
    users: {
      type: new GraphQLNonNull(new GraphQLList(new GraphQLNonNull(User))),
      resolve: async (_parent, _args, context, info: GraphQLResolveInfo) => {
        const relations = getSelectedRelations(info);
        return context.prisma.user.findMany({
          include: {
            userSubscribedTo: relations.userSubscribedTo,
            subscribedToUser: relations.subscribedToUser,
          }
        })
      },
    },
    user: {
      ...
      resolve: async (_parent, args: { id: string }, context, info: GraphQLResolveInfo) => {
        try {
          const relations = getSelectedRelations(info);
...
const getSelectedRelations = (info: GraphQLResolveInfo): {userSubscribedTo: boolean; subscribedToUser: boolean} => {
  const relations = { userSubscribedTo: false, subscribedToUser: false };
  const parsed = parse(info.operation.loc!.source.body);
  visit(parsed, { Field(node: FieldNode) {
    if (node.name.value === 'userSubscribedTo') {
      relations.userSubscribedTo = true;
    }
    if (node.name.value === 'subscribedToUser') {
      relations.subscribedToUser = true;
    }
  },
 });
 return relations;
